...
const showTiles = async () => {
  ...
  try {
    ...
    const {
      data: { tiles },
    } = await axios.get('https://tiles-todo.netlify.app/api/tiles');
...
tilesDOM.addEventListener('click', async (e) => {
  ...
  if (el.parentElement.classList.contains('delete-btn')) {
    ...
    try {
      ...
      await axios.delete(`https://tiles-todo.netlify.app/api/tiles/${id}`);
      // re-fetch all tiles
      showTiles();
    } catch (error) {
      console.log(error);
...
formDOM.addEventListener('submit', async (e) => {
  ...
  try {
    ...
    await axios.post('https://tiles-todo.netlify.app/api/tiles', { name });
    // re-fetch tiles
    showTiles();
    // add correct alerts
    tileInputDOM.value = '';
